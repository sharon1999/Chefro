...
import { ArrowLeft, Utensils, Hash } from "lucide-react";
...
export default async function CategoryPage({ params }) {
  ...
  const res = await getMealsByCategory(decodedCategory);
    ...
    <div className="container mx-auto px-4 py-8 space-y-8">
      ...
      <div className="flex items-center gap-4">
        <div className="w-16 h-16 relative rounded-full overflow-hidden border-2 border-primary/20 shadow-sm bg-background flex items-center justify-center shrink-0">
          <Hash className="w-8 h-8 text-primary/80" />
        </div>
      ...
      {meals.length === 0 ? (
        <div className="text-center py-12 bg-muted/30 rounded-2xl border">
          ...
          <h3 className="text-xl font-medium text-muted-foreground">No dishes found for this category.</h3>
        </div>
      ) : (
